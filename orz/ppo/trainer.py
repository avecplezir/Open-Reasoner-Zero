...
class RayPPOTrainer(BaseTrainer):
    ...
    async def make_experience(self, all_inputs: Union[Tuple[str, dict], List[Tuple[str, dict]]], **generate_kwargs):
        ...
        all_opponents_prompts, all_opponents_extras = (
            self._create_opponents_prompts(all_prompts, all_extras, bos_token))

        logger.info(f'len of all_opponents_prompts and aug_opponents_extras: {len(all_opponents_prompts)} {len(all_opponents_extras)}')

        # 1. generate sequences and inference, calculate values, log probs, rewards, kl divergence, generate sequences via vllm engines
        opponents_outputs = await self._distributed_generate(all_opponents_prompts, all_opponents_extras, teacher=True, desc="Generate complimentary teacher sequences via vllm engines", **generate_kwargs)

        # skip when data is not enough
        if len(outputs) <= 0:
            return

        assert len(all_opponents_prompts) == len(opponents_outputs), f"generate objects number must be equal to all inputs number {len(all_opponents_prompts)}, {len(opponents_outputs)}"
        ...
        if self.cfg.use_compute_reward_fn:
            async with Timer("Calculate custom rewards"):
                ...
                all_opponents_prompts, opponents_outputs, _, opponents_custom_rewards, _, _, _, opponents_final_answers, _, _, _, _ = await reward_fn(
                    all_opponents_prompts, opponents_outputs, all_extras, prefix='teacher/')

        async with Timer("Generating verification responses"):
            adv_prompts, adv_extras, adv_index_groups = self._build_adversarial_student_prompts(
                opponents_outputs,
                all_opponents_prompts,
                all_opponents_extras,
                bos_token,
                opponents_final_answers,
            )
        ...
        logger.info(f"Generated {len(adv_prompts)} verifier responses for {len(adv_index_groups)} adversarial groups")
        assert len(adv_initial_teacher_scores) == len(adv_index_groups) * self.cfg.adv_n_samples_per_prompt, (
        ...
        if not self.cfg.adv_teacher_get_correct_reward and not self.cfg.repeat_randomly_once:
            for g in range(len(adv_index_groups)):
                if len(adv_index_groups[g]) == 2:
                    idx1, idx2 = adv_index_groups[g]
